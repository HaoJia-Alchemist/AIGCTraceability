...
class BaseProcessor(ABC):
    ...
    def __init__(
            self,
            config,
            model, train_loader=None, val_loader=None, optimizer=None,
    ...
    def save_model(self):
        ...
        unwrap_model.save(osp.join(self.config["logging"]["log_dir"], "best_model"))

        logger.info(
            "Saved model to {}".format(osp.join(self.config["logging"]["log_dir"], "best_model")))

    def load_model(self, model_path):
        unwrap_model = self.accelerator.unwrap_model(self.model)
        unwrap_model.load(model_path)
