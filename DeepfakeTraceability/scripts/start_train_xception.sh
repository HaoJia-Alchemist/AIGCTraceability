...
# Xception模型训练配置
echo "开始Xception模型训练..."
echo "配置文件: ./config/configs_dir/xception.yaml"

# 设置环境变量并启动加速器（动态添加多GPU参数）
CUDA_VISIBLE_DEVICES="$GPU_IDS" accelerate launch $MULTI_GPU_ARG --num_processes "$NUM_PROCESSES" train.py --config "./config/configs_dir/xception.yaml" --opt task_name='xception_train' dataset.sampler='softmax_triplet'